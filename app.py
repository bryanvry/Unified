...
with c2:
    master_xlsx = st.file_uploader("Upload MASTER (XLSX) — SG/Nevada/Breakthru", type=["xlsx"])
with c3:
    pricebook_csv = st.file_uploader("Upload PRICEBOOK (CSV) — SG/Nevada/Breakthru", type=["csv"])
    ...
    if not inv_files:
        ...
        if slug == "unified":
            if not pricebook_csv:
                ...
                gs1, gs2_posupload, unmatched, _ = _process_unified(pricebook_csv, inv_files)
                st.session_state["outputs"] = {
                    "invoice_items_csv": None,
                    "updated_master_xlsx": None,
                    "cost_changes_csv": None,
                    "not_in_master_csv": _df_to_csv_bytes(unmatched),
                    "missing_pack_csv": None,
                    "pos_update_xlsx": _dfs_to_xlsx_bytes({"POS Update": gs2_posupload}),
                    "missing_in_pricebook_csv": None,
                    "gs1_csv": _df_to_csv_bytes(gs1),
                }
            ...
            # --- Breakthru-only optional UPC backfill from Master via Item Number -> Master["Invoice UPC"] -> Master["Full Barcode"]
            ...
            # Always normalize UPC for writing & matching downstream
            if "UPC" in invoice_items.columns:
                invoice_items["UPC"] = invoice_items["UPC"].astype(str).apply(_norm12_or_blank)
            ...
            # Generic SG/Nevada/Breakthru downstream (Unified is separate)
            if slug in ("southern_glazers", "nevada_beverage", "breakthru") and master_xlsx is not None and pricebook_csv is not None and not invoice_items.empty:
...
if outs is not None:
    ...
    with d1:
        ...
        if outs["invoice_items_csv"] is not None:
            # Breakthru gets a different default filename, but same bytes
            fname = "bthru_invoice_items.csv" if vendor_label == "Breakthru" else "invoice_items.csv"
            st.download_button("⬇️ " + fname, data=outs["invoice_items_csv"], file_name=fname, mime="text/csv")
